    ...
    return (
        <div className="max-w-screen-xl mx-auto px-6 py-8 text-white">
            ...
            <div className='mb-4 flex items-center space-x-6 mb-8'>
                <img src={coin.image} alt={coin.symbol} className="w-75 h-75 object-cointain rounded-xl border-2 border-gray-600" />
                <div>
                    <h1 className="text-3x font-bold">{coin.name} ({coin.symbol.toUpperCase()})</h1>
                    <p className={`text-2xl mt-2 ${priceColor}`}>Live: ${(Number(livePrice).toFixed(2)).toLocaleString('en-US', {
                        style: 'currency',
                        currency: 'USD',
                    })}</p>
                </div>
            ...
            <div className='grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 text-sm'>
                <h1>Current Price: {(coin.current_price).toLocaleString('en-US', {style: 'currency',currency: 'USD',})}</h1>
                <h1>Market Cap: {(coin.market_cap).toLocaleString('en-US', {style: 'currency',currency: 'USD',})}</h1>
                <h1>Market Cap Rank: {coin.market_cap_rank}</h1>
                <h1>Valuation: {(coin.fully_diluted_valuation).toLocaleString('en-US', {style: 'currency',currency: 'USD',})}</h1>
                <h1>Total Daily Volume: {coin.total_volume}</h1>
                <h1>High of Day: {(coin.high_24h).toLocaleString('en-US', {style: 'currency',currency: 'USD',})}</h1>
                <h1>Low of Day: {(coin.low_24h).toLocaleString('en-US', {style: 'currency',currency: 'USD',})}</h1>
                <h1>Daily Price Chnage: {(coin.price_change_24h).toLocaleString('en-US', {style: 'currency',currency: 'USD',})}</h1>
                <h1>Price Change Percentage: {coin.price_change_percentage_24h}%</h1>
                <h1>24 Hour Market Cap Change: {(coin.market_cap_change_24h).toLocaleString('en-US', {style: 'currency',currency: 'USD',})}</h1>
                ...
                <h1>All Time High: {(coin.ath).toLocaleString('en-US', {style: 'currency',currency: 'USD',})}</h1>
                <h1>All Time High Change Percentage: {coin.ath_change_percentage}%</h1>
                <h1>All Time High Date: {coin.ath_date}</h1>
                <h1> All Time Low: {(coin.atl).toLocaleString('en-US', {style: 'currency',currency: 'USD',})}</h1>
